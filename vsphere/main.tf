...
# datastore에 저장되어있는 template 이름
# 템플릿을 원활하게 사용하려면 vmware tools or open vm tools의 설치가 필요하다.(vmware 환경)
...
# [datastore] iso/ubuntu-20.04.5-desktop-amd64.iso
  ...
  clone {
    ...
    customize {
      ...
      network_interface {
        ...
        ipv4_netmask = 26 # 생성될 ip의 netmask
      }
